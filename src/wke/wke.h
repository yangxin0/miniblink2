...
// restyle or hide elements before a capture. True on success. Port extension.
WKE_API bool wkeInsertCSS(wkeWebView webView, const utf8* css);
...
WKE_API const utf8* wkeGetTextForSelector(wkeWebView webView,
                                          const char* selector);
// outerHTML of the first match (element + markup) — extract a fragment to
// re-parse, vs wkeGetTextForSelector (text) / wkeGetSource (whole doc). "" if no
...
WKE_API bool wkeWaitForVisibleSelector(wkeWebView webView, const char* selector,
                                       int timeoutMs);
// The inverse: wait until the first match is NOT visible (gone or hidden) — the
...
WKE_API const utf8* wkeGetCookieValue(wkeWebView webView, const utf8* name);
...
WKE_API void wkeSetCookie(wkeWebView webView, const utf8* url,
                          const utf8* cookie);
// localStorage access for the document's origin (inject an auth token, read SPA
// state). wkeGetLocalStorage returns the value ("" if absent / no storage), owned
// by the view; wkeSetLocalStorage stores key=value (true on success). Needs a
// real origin (an http(s) base), not about:blank. Port extension.
...
WKE_API void wkeClearStorage(wkeWebView webView);
